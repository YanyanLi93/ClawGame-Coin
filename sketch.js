...
function draw() {
  ...
  for (let i = 0; i < balls.length; i++) {
    ...
    if (
      ...
      if (player.x >= 570 && grabbedBall) {
  // 50% chance to make the ball fall
  if (random(1) <= 0.8) { // 50% chance
    grabbedBall = false; // Reset the grabbedBall flag when the ball falls
  }
    }
  }
  }
...
if (player.x >= 570 && grabbedBall && !imageProcessed) {
    ...
    if (selectedImage === images[4] || selectedImage === images[6]) {
      score += 30; // Add 3 points for image5 or image7
    } else if (
      selectedImage === images[0] ||
      selectedImage === images[1] ||
      selectedImage === images[9]
    ) {
      score += 20; // Add 2 points for image1, image2, or image10
    } else if (
      ...
      score += 10; // Add 1 point for image3, image6, image8, or image9
    }
  ...
  if (coin >= 0) {
    fill(0);
    textSize(32);
    text("Score: " + score, width / 2 + 150, 50); 
    fill(0);
    textSize(32);
    text("Coins: " + coin, width / 2 - 270, 50);
  }

  // Once the timer runs out, display "Game Over" and the score
  if (coin <=0 && player.x > 570) {
    player.color = "0";
    player.stroke = "0";
    clawL.color = "0";
    clawL.stroke = "0";
    ...
    stroke(124, 75, 146);
    strokeWeight(10)
    rect(width / 2-50, height / 2 - 120, 350, 180)
    textAlign(CENTER);
    fill(0);
    noStroke();
    textSize(48);
    text("No coins!" , width / 2-50, height / 2 - 150);
